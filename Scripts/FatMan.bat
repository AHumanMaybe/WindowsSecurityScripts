...
REM Add Active Directory
echo Enabling Active Directory
dism /online /enable-feature /featurename:RSATClient-Roles-AD
dism /online /enable-feature /featurename:RSATClient-Roles-AD-DS
dism /online /enable-feature /featurename:RSATClient-Roles-AD-DS-SnapIns

REM Configure File Explorer
echo Configuring settings for file explorer to view hidden files, protected system files, and file extensions
...
echo after this the script will REALLY start
pause

REM FORENSICSSSS
echo DO FORENSICS FIRST BEFORE USING THIS!!!!!
echo DO FORENSICS FIRST BEFORE USING THIS!!!!!
echo DO FORENSICS FIRST BEFORE USING THIS!!!!!
echo DO FORENSICS FIRST BEFORE USING THIS!!!!!
echo DO FORENSICS FIRST BEFORE USING THIS!!!!!
...
REM PROGRAMS
echo TRY AND GET AROUND 25% BEFORE USE!!!!!!!
echo TRY AND GET AROUND 25% BEFORE USE!!!!!!!
echo TRY AND GET AROUND 25% BEFORE USE!!!!!!!
echo TRY AND GET AROUND 25% BEFORE USE!!!!!!!
echo TRY AND GET AROUND 25% BEFORE USE!!!!!!!
...
REM Bit Locker Drive Encryption
echo turning on
...
echo Before proceeding please install ntrights.exe from ntrights.zip folder
pause
...
if exist C:\Windows\System32\ntrights.exe (
	...
	for %%a in (%remove%) do (
			...
			ntrights -U %%a -R SeTimeZonePrivilege
			ntrights -U %%a -R SeTcbPrivilege
			ntrights -U %%a -R SeRemoteShutdownPrivilege
			ntrights -U %%a -R SeChangeNotifyPrivilege
		...
		ntrights -U "Users" -R SeLoadDriverPrivilege
...
REM Audits
echo Success and Failure for all categories
auditpol /set /category:* /success:enable
auditpol /set /category:* /failure:enable
echo auditing success and failure complete
...
REM IIS
set /p iisChk="Enable IIS (y/n)"
if %iisChk%==y (
	echo Enabling IIS...
set featuresEnable=IIS-WebServerRole IIS-WebServer IIS-CommonHttpFeatures IIS-HttpErrors IIS-HttpRedirect IIS-ApplicationDevelopment IIS-NetFxExtensibility IIS-NetFxExtensibility45 IIS-HealthAndDiagnostics IIS-HttpLogging IIS-LoggingLibraries IIS-RequestMonitor IIS-HttpTracing IIS-Security IIS-URLAuthorization IIS-RequestFiltering IIS-IPSecurity IIS-Performance IIS-HttpCompressionDynamic IIS-WebServerManagementTools IIS-ManagementScriptingTools IIS-IIS6ManagementCompatibility IIS-Metabase IIS-HostableWebCore IIS-StaticContent IIS-DefaultDocument IIS-DirectoryBrowsing IIS-WebDAV IIS-WebSockets IIS-ApplicationInit IIS-ASPNET IIS-ASPNET45 IIS-ASP IIS-CGI IIS-ISAPIExtensions IIS-ISAPIFilter IIS-ServerSideIncludes IIS-CustomLogging IIS-BasicAuthentication IIS-HttpCompressionStatic IIS-ManagementConsole IIS-ManagementService IIS-WMICompatibility IIS-LegacyScripts IIS-LegacySnapIn IIS-FTPServer IIS-FTPSvc IIS-FTPExtensibility TFTP TelnetClient TelnetServer
for %%a in (%featuresEnable%) do dism /online /enable-feature /featurename:%%a
dism /online /enable-feature /featurename:IIS /all
...
echo Disabling Windows features...
set features=TFTP TelnetClient TelnetServer
for %%a in (%features%) do dism /online /disable-feature /featurename:%%a
echo Disabled Windows features


REM COMPUTER MANAGEMENT CHECK

start compmgmt.msc /wait
echo LOOK FOR ANY HIDDEN USERS!!!!
echo DELETE SHARES, CHECK CURRENTLY USED SHARES AND CHECK ANY OPEN SHARES/SHARED FILES OUT

pause

REM Local Security Policy

color O5
	echo Windows Defender Enabled in gpedit
::Check this works!!!
...
reg delete "HKLM\SOFTWARE\Policies\Microsoft\Windows Defender\Real-Time Protection" /v DisableRealtimeMonitoring
	echo PrintNightmare Protection
...
reg ADD "HKCU\CONTROL PANEL\DESKTOP" /v ScreenSaveUsePassword /t REG_DWORD /d 1 /f
	echo Allow system to be shutdown without having to log on disabled(IF LOSE POINTS CHANGE THIS FROM 0 TO 1)
reg ADD "HKLM\Software\Microsoft\Windows\CurrentVersion\Policies\System" /v ShutdownWithoutLogon /t REG_DWORD /d 0 /f
	pause
	echo Autorun cdrom disabled
reg add "HKLM\SYSTEM\CurrentControlSet\Services\cdrom" /v AutoRun /t REG_DWORD /d 0 /f
	echo Administrator accounts must not be enumerated during elevation
reg ADD "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\CredUI" /v EnumerateAdministrators /t REG_DWORD /d 1 /f
	echo Clear Page File At Shutdown enabled
reg ADD "HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management" /v ClearPageFileAtShutdown /t REG_DWORD /d 1 /f
	echo Allow automatic administrative logon to disabled
reg ADD "HKLM\Software\Microsoft\Windows NT\CurrentVersion\Setup\RecoveryConsole" /v SecurityLevel /t REG_DWORD /d 0 /f
	echo Do not store LAN Manager hash value on next password change setting to enabled
...
reg ADD "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" /v AllocateFloppies /t REG_DWORD /d 1 /f
	echo Limiting local account use of blank passwords to console logon only
reg add "HKLM\SYSTEM\CurrentControlSet\Control\Lsa" /v LimitBlankPasswordUse /t REG_DWORD /d 1 /f
	echo enabling windows auto updates
reg add "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Auto Update" /v AUOptions /t REG_DWORD /d 0 /f
	echo Preventing users from installing printer drivers
reg add "HKLM\SYSTEM\CurrentControlSet\Control\Print\Providers\LanManPrintServices" /v AddPrinterDrivers /t REG_DWORD /d 1 /f
 	echo restricting CD-ROM access to locally logged-on user only
reg add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" /v AllocateCDRoms /t REG_SZ /d 1 /f
	echo setting CTRL ALT DEL
reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System" /v DisableCAD /t REG_DWORD /d 0 /f
	echo Setting Do not display last user name
reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System" /v DontDisplayLastUserName /t REG_DWORD /d 0 /f
echo enabling do not allow drive redirection
::fdisablecdm = do not allow drive redirection
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows NT\Terminal Services" /v fDisableCdm /t REG_DWORD /d 1 /f
echo disabling allow automatic administrative logon
reg add "HKLM\Software\Microsoft\Windows NT\CurrentVersion\Setup\RecoveryConsole" /v SecurityLevel /t REG_DWORD /d 0 /f
echo most seen point givers for policies completed

REM UAC settings
reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System" /v EnableLUA /t REG_DWORD /d 1 /f
reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System" /v ConsentPromptBehaviorAdmin /t REG_DWORD /d 2 /f
reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System" /v PromptOnSecureDesktop /t REG_DWORD /d 1 /f
echo UAC set to max settings
pause

color 0B

REM User Password stuff
...
echo set password policy: Password policy must meet complexity - enable
echo Set password policy: Store passwords using reversible encryption - disabled
echo ###########################DOUBLE CHECK THESE######################################
echo disable ctrl alt del - disabled
echo dont display last user name - enabled
echo restrict cd rom access to locally logged on users only - enabled
echo addprinterdrivers restrcted
echo if these values are not set to the given settings then the local security values were not changed by this script and there is another issue at play...
echo ###################################################################################
echo CHECK USER RIGHTS ASSIGNMENT FOR INDIVIDUAL USERS!!!!!
echo secpol.msc will be started for manual changes/checks
...
echo look for suspicous tasks
echo look for weird names or nmap stuff
echo delete any unwanted tasks
echo find source of sus tasks
start taskschd.msc /wait
pause 
...
REM GPEDIT
start rsop.msc /wait
echo Check for configured settings!!
pause
start gpedit.msc /wait
echo ENABLE USER MANAGMENT

pause

REM Programs in Startup
start msconfig /wait
echo ONLY: VMware stuff, Malware bytes (if downloaded)
echo if ANYTHING else is listed find file location and google to check if malicious
pause
start taskmgr /wait
echo CHECK BACKGROUND TASKS AND STARTUP APPLICATIONS HERE TOO!
pause


::###############################################################################################
::REM User Passwordsss

::echo Setting passwords to expire, and making all passwords: CyberPatriot20! 
::echo Please change the main account password after script
::for /f "tokens=*" %%a in ('type %path%resources\users.txt') do (
	::net user "%%a" "CyberPatriot20!"
	::C:\Windows\System32\wbem\wmic UserAccount where Name="%%a" set PasswordExpires=True
::)
::echo Made passwords expire, and set passwords to CyberPatriot20!

::############################################################################################
::REM Secure Possible Script Vulnerabilities 
::echo Securing Possible Script Vulnerabilities 
...
::
	...
	REM Prevent unauthenticated RPC connections
...
::##########################################################
::following causes issues with running programs in windows, do not run


::echo Prioritize ECC Curves with longer keys - IISCrypto (recommended options)
...
echo DO FORENSICS
echo GOODLUCK
echo empty temp folder
echo check for stuff you know, nothing is as hard as it seems
echo check through secpol and everything else for things the script missed 
echo remember things arent as complicated as you think
echo let updates finish and restart!!!!
echo dont forget to check for firefox points
echo DONT FORGET USER AUDITING
echo all that should be left to do is User auditing, deleting unwanted programs/files, and Updates
echo DONT FORGET TO DO A RESTART AFTER COMPLETING SCRIPT!!!!
pause